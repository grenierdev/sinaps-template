...
class Environment {
	...
	render(name, data, env) {
		var compiled = this._compileNameAndCache(name, env);
	...
	_compileNameAndCache (name, env) {
		name = path.normalize(name);
		if (env && env.path) {
			name = path.normalize(path.join(path.dirname(env.path), name));
		}
		...
		if (typeof this.cached[name] === 'undefined') {
			...
			this.cached[name] = this.compileString(source.buffer, _.extend({path: name}, env));
		}
		return this.cached[name];
	}

	renderString (str, data, env) {
		var compiled = this.compileString(str, env);
		var result = compiled(data);
		return result.output;
	}

	contextString (str, data, env) {
		var compiled = this.compileString(str, env);
		var result = compiled(data);
		return result.context;
	}

	compileString (str, env) {
		var compiled;
		try {
			compiled = this._compileFromString(str, env);
		} catch (e) {
	...
	_compileFromString (str, env) {
		...
		code.push(`var env = ${JSON.stringify(env)};`);
		walkAST(ast[0]);
			...
			for (var i = 0, l = tree.length; i < l; ++i) {
				...
				switch(node.type) {
					...
					case 'TemplateExtend':
						...
						break;
					case 'TemplateImport':
						code.push(`context.set("${node.as.name}", this.getContext("${node.template.value}", env));`);
						break;
					case 'TemplateMacro':
						...
						break;
					case 'TemplateBlock':
						code.push(`var current = (function (context) { return function (parent) { "use strict"; var output = ""; context.set('parent', parent || noop); `);
						walkAST(node.children, [node].concat(parents));
